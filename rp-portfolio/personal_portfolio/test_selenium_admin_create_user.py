...
def test_create_user_password_similar_to_username():
    ...
    time.sleep(3)
    driver.find_element_by_name("username").send_keys("eshwar")
    driver.find_element_by_name("password1").send_keys("Eshwar7272")
    driver.find_element_by_name("password2").send_keys("Eshwar7272")
    driver.find_element_by_xpath("//input[@type='submit' and @value='Save']").click()
    ermsg1 = driver.find_element_by_class_name("errornote").text
    ermsg2 = driver.find_element_by_xpath("//*[contains(text(), 'The password is too similar to the username.')]").text
    assert 'Please correct the error below.' == ermsg1 and 'The password is too similar to the username.' == ermsg2 
    driver.close()  
...
def test_create_user_password_less_than_8_characters():
    ...
    assert 'Please correct the error below.' == ermsg1 and 'This password is too short. It must contain at least 8 characters.' == ermsg2 
    driver.close()
...
def test_create_user_password_is_numeric():
    ...
    assert 'Please correct the error below.' == ermsg1 and 'This password is entirely numeric.' == ermsg2 
    driver.close()
...
def test_create_user_password_is_different_from_confirm_password():
    ...
    driver.find_element_by_xpath("//input[@type='submit' and @value='Log in']").click()
    time.sleep(3)
    driver.find_element_by_name("username").send_keys("eshwar123")
    ...
    assert 'Please correct the error below.' == ermsg1 and "The two password fields didn't match." == ermsg3 
    driver.close()
...
def test_create_user_username_empty():
    ...
    ermsg1 = driver.find_element_by_class_name("errornote").text
    ermsg2 = driver.find_element_by_xpath("//*[contains(text(), 'This field is required.')]").text
    assert 'Please correct the error below.' == ermsg1 and "This field is required." == ermsg2  
    driver.close()
...
def test_create_user_confirmpassword_empty():
    ...
    driver.find_element_by_xpath("//input[@type='submit' and @value='Log in']").click()
    time.sleep(3)
    driver.find_element_by_name("username").send_keys("testing2k00")
    driver.find_element_by_name("password1").send_keys("Str0ngP@ww0rd")
    driver.find_element_by_name("password2").send_keys("")
    driver.find_element_by_xpath("//input[@type='submit' and @value='Save']").click()
    ermsg1 = driver.find_element_by_class_name("errornote").text
